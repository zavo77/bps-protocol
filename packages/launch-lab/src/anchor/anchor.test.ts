...
function mockApi(opts?: {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string) => {
      ...
      return { ok: true, json: async () => ({ quotes: [{ bid: opts?.bid ?? '323.8', ask: opts?.ask ?? '324.0' }] }) };
    }),
  );
}
...
describe('resolveAnchor (fail closed)', () => {
  ...
  it.each([
    ...
    ['API address mismatch', { client: {}, api: { address: '0x0000000000000000000000000000000000000001' } }],
