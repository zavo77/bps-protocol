...
export async function resolveAnchor(client: PublicClient, opts?: { bypassCache?: boolean }): Promise<AnchorVerification> {
  if (!opts?.bypassCache && cache && Date.now() - cache.at < CACHE_TTL_MS && cache.value.status === 'verified') {
    return cache.value;
  }
  ...
  try {
    ...
    const list: RawAsset[] = Array.isArray(assetsJson)
      ...
      : (((assetsJson as Record<string, unknown>).assets ?? (assetsJson as Record<string, unknown>).results ?? []) as RawAsset[]);
  ...
  if (apiAddress !== GOOGL_ADDRESS) {
    return mismatch(`Official GOOGL address ${apiAddress} does not match configured ${GOOGL_ADDRESS}.`);
  }
