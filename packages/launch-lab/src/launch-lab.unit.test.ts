...
import { MissingEnvError, requireEnv, readServerFlags, getFeePreset, FEE_PRESETS } from './config/index';
...
import { validateMetadataInput, isBroadcastableTokenUri, localPreviewMetadata, type MetadataInput } from './metadata/index';
...
describe('config', () => {
  ...
  it('DYNAMIC_PROTECTION is disabled with a reason', () => {
    ...
    expect(FEE_PRESETS.filter((x) => x.enabled).map((x) => x.id)).toEqual(['BALANCED_1', 'CREATOR_2', 'DEGEN_3']);
  });
...
describe('validation', () => {
  ...
  it('signed envelope requires chain 4663 and hex hash', () => {
    ...
    expect(() => signedRequestSchema.parse({ ...msg, message: { ...msg.message, chainId: 1 } })).toThrow();
  });
});
...
describe('metadata validation', () => {
  ...
  it('local preview is never broadcastable', () => {
    ...
    expect(
      isBroadcastableTokenUri({ imageCid: 'bafyimagecid1234', metadataCid: 'bafymetacid1234', tokenUri: 'ipfs://bafymetacid1234', provider: 'pinata' }),
    ).toBe(true);
  });
});
...
describe('launch params (pure build, no network)', () => {
  ...
  it('refuses the disabled DYNAMIC_PROTECTION preset', () => {
    expect(() => buildLaunchParams({ ...base, feePreset: 'DYNAMIC_PROTECTION' })).toThrow(/disabled/i);
  });
});
