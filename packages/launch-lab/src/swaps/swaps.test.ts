...
import { GOOGL_ADDRESS as ANCHOR_ADDRESS } from "../config/index";
...
function ctx(anchorIsCurrency0: boolean): LabPoolContext {
  const currency0 = anchorIsCurrency0 ? ANCHOR_ADDRESS : TOKEN;
  const currency1 = anchorIsCurrency0 ? TOKEN : ANCHOR_ADDRESS;
  return {
...
function quote(direction: "buy" | "sell", anchorIsCurrency0: boolean): SwapQuote {
  return {
    direction,
    amountInWei: "1000000000000000000",
    amountOutWei: "500000000000000000",
    gasEstimate: "400000",
    zeroForOne: directionToZeroForOne(direction, anchorIsCurrency0),
    poolKey: ctx(anchorIsCurrency0).poolKey,
    quotedAt: Date.now(),
  };
}
...
describe("token ordering", () => {
  ...
  it("input/output currency follow the resolved ordering, not an assumption", () => {
    ...
    const buyG0 = buildSwapTransaction({
      ctx: ctx(true),
      quote: quote("buy", true),
      slippageBps: 100,
    });
    expect(buyG0.inputCurrency).toBe(ANCHOR_ADDRESS);
    expect(buyG0.outputCurrency).toBe(TOKEN);
    // GOOGL is currency1: a buy still spends GOOGL (now currency1).
    const buyG1 = buildSwapTransaction({
      ctx: ctx(false),
      quote: quote("buy", false),
      slippageBps: 100,
    });
    expect(buyG1.inputCurrency).toBe(ANCHOR_ADDRESS);
    expect(buyG1.outputCurrency).toBe(TOKEN);
    // A sell spends the token in both orderings.
    const sell = buildSwapTransaction({
      ctx: ctx(true),
      quote: quote("sell", true),
      slippageBps: 100,
    });
    expect(sell.inputCurrency).toBe(TOKEN);
    expect(sell.outputCurrency).toBe(ANCHOR_ADDRESS);
  });
});
