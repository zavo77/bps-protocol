...
export function validateMetadataInput(input: MetadataInput): string[] {
  const errors: string[] = [];
  if (!/^[\x20-\x7E]{1,48}$/.test(input.tokenName)) errors.push('Token name must be 1-48 printable ASCII characters.');
  if (!/^[A-Z0-9]{1,12}$/.test(input.tokenSymbol)) errors.push('Ticker must be 1-12 characters A-Z or 0-9.');
  if (input.tokenDescription.length < 1 || input.tokenDescription.length > 600) errors.push('Description must be 1-600 characters.');
  if (/[<>]/.test(input.tokenDescription) || /[<>]/.test(input.tokenName)) errors.push('HTML characters are not allowed.');
  if (!ALLOWED_MIME.includes(input.imageMime)) errors.push('Image must be PNG, JPEG, or WebP.');
  if (input.imageBytes.length === 0 || input.imageBytes.length > MAX_IMAGE_BYTES) errors.push('Image must be 1 byte to 4 MB.');
  const magic = MAGIC[input.imageMime];
  if (magic && !magic.every((b, i) => input.imageBytes[i] === b)) errors.push('Image bytes do not match the declared type.');
  if (!/^[A-Za-z0-9._-]{1,80}$/.test(input.imageFilename)) errors.push('Image filename contains unsafe characters.');
  return errors;
}
...
export async function uploadMetadataViaPinata(input: MetadataInput): Promise<MetadataUploadResult> {
  ...
  form.append(
    'file',
    new File([input.imageBytes as unknown as BlobPart], input.imageFilename, { type: input.imageMime }),
  );
...
export function isBroadcastableTokenUri(result: MetadataUploadResult | null | undefined): boolean {
  return !!result && result.provider === 'pinata' && result.tokenUri.startsWith('ipfs://') && result.metadataCid.length > 10;
}
