...
describe("decodeAndVerifyReceipt", () => {
  it("accepts a matching receipt", async () => {
    const r = await decodeAndVerifyReceipt(
      clientWith(),
      manifest,
      TOKEN,
      receiptWith([createLog()]),
    );
  ...
  it("hard-fails on wrong numeraire", async () => {
    ...
    const r = await decodeAndVerifyReceipt(
      ...
      manifest,
      TOKEN,
      receiptWith([createLog(wrongNumeraire)]),
    );
    expect(r.matchesManifest).toBe(false);
  ...
  it("hard-fails on wrong on-chain name or supply", async () => {
    const r = await decodeAndVerifyReceipt(
      clientWith({ name: "NOTPRINT" }),
      manifest,
      TOKEN,
      receiptWith([createLog()]),
    );
    expect(r.matchesManifest).toBe(false);
    const r2 = await decodeAndVerifyReceipt(
      clientWith({ supply: 1n }),
      manifest,
      TOKEN,
      receiptWith([createLog()]),
    );
