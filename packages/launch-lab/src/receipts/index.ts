...
export async function decodeAndVerifyReceipt(
  ...
  const ev = created.find(
    (l) => l.address.toLowerCase() === manifest.transactionTarget.toLowerCase(),
  );
  ...
  if (!ev) {
    ...
    if (
      initializerExpected &&
      args.initializer.toLowerCase() !== initializerExpected.toLowerCase()
    ) {
  ...
  if (tokenAddress !== "0x0000000000000000000000000000000000000000") {
    try {
      ...
      if (name !== manifest.tokenName)
        mismatches.push(`Token name "${name}" != "${manifest.tokenName}".`);
      if (symbol !== manifest.tokenSymbol)
        mismatches.push(`Token symbol "${symbol}" != "${manifest.tokenSymbol}".`);
    ...
    try {
      ...
      if (
        migrator &&
        !assetData.some((a) => typeof a === "string" && a.toLowerCase() === migrator)
      ) {
