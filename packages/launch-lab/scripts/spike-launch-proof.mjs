...
try {
  ...
  for (const r of required) {
    ...
    if (!m || m.code === "EMPTY" || m.code === "ZERO_ADDRESS")
      fail(`required module ${r} missing/empty on 4663`);
  ...
  if (apiAddr !== GOOGL_EXPECTED)
    throw new Error(`GOOGL address mismatch: api=${apiAddr} expected=${GOOGL_EXPECTED}`);
  ...
  console.log(
    `OK GOOGL verified (${gName}, ${gDecimals}dp, mid $${mid.toFixed(2)}, multiplier ${multiplier})`,
  );
  ...
  report.beneficiaries = beneficiaries.map((b) => ({
  ...
  const buildParams = (variant) => {
    const b = new MulticurveBuilder(CHAIN_IDS.ROBINHOOD)
      .tokenConfig({
        ...
        name: "PRINT",
        symbol: "PRINT",
        tokenURI: "ipfs://spike-placeholder-not-final",
      })
      ...
      .withCurves({
        ...
        curves: [
          {
            marketCap: { start: START_FDV_USD, end: 1_000_000 },
            numPositions: 11,
            shares: (WAD * 60n) / 100n,
          },
          {
  ...
  for (const variant of ["rehype", "standard"]) {
    try {
      ...
      const calldataHash = keccak256(
        sim.createParams && sim.createParams.data ? sim.createParams.data : "0x",
      );
      ...
      console.log(
        `SIMULATION_OK [${variant}] token=${sim.tokenAddress} poolId=${sim.poolId} gas=${sim.gasEstimate}`,
      );
    } catch (e) {
      report.simulation[variant] = {
        status: "FAILED",
        error: redact(e?.shortMessage ?? e?.message ?? String(e)).slice(0, 600),
      };
      console.error(
        `SIMULATION FAILED [${variant}]:`,
        redact(e?.shortMessage ?? e?.message ?? String(e)).slice(0, 600),
      );
    }
  }

  const ok =
    report.simulation.rehype?.status === "SIMULATION_OK" ||
    report.simulation.standard?.status === "SIMULATION_OK";
...
writeFileSync(
  path.join(outDir, "SPIKE_LAUNCH_PROOF.json"),
