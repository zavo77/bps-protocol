...
// ---- historical-fixture + safety flags ----
// mode may be "live" (fresh provider-backed packet) or "historical-fixture"; in BOTH cases the packet
// must remain NON-EXECUTABLE — the economics approval permits preparation only.
ok("mode is live or historical-fixture, packet+policy agree, and executable=false in both",
  ((a.meta.mode === "live" && a.meta.live === true && policy.mode === "live" && policy.live === true) ||
   (a.meta.mode === "historical-fixture" && a.meta.live === false && policy.mode === "historical-fixture" && policy.live === false))
  && a.meta.executable === false && policy.executable === false, `mode=${a.meta.mode}`);
...
ok("all four safety flags false in packet", a.safetyFlags.broadcastReady === false && a.safetyFlags.liveWritesApproved === false && a.safetyFlags.executionAuthorized === false && a.safetyFlags.fundingAuthorized === false);
ok("all four safety flags false in policy", policy.safetyFlags.broadcastReady === false && policy.safetyFlags.liveWritesApproved === false && policy.safetyFlags.executionAuthorized === false && policy.safetyFlags.fundingAuthorized === false);
for (const m of ["robinhood-mainnet.canary", "robinhood-mainnet.dryrun"]) { const mm = rj(`manifests/${m}.json`); if (m.includes("canary")) ok("manifest broadcastReady=false & liveWritesApproved=false", mm.broadcastReady === false && mm.liveWritesApproved === false); }
...
let mut = true; for (const [pth, fn] of [["meta.capPriceMicroUsd", o => o.meta.capPriceMicroUsd = "1"], ["safetyFlags.broadcastReady", o => o.safetyFlags.broadcastReady = true], ["fundingPlan.executable", o => o.fundingPlan.executable = true], ["immediateTransactions[0].dataKeccak", o => o.immediateTransactions[0].dataKeccak = "0x01"], ["provenance.repoHead", o => o.provenance.repoHead = "x"], ["addressGuards.canaryTokenSymbol", o => o.addressGuards.canaryTokenSymbol = "BPS"]]) { const c = JSON.parse(JSON.stringify(a)); fn(c); if (recomputeFull(c) === a.meta.fullReviewArtifactDigest) mut = false; }
