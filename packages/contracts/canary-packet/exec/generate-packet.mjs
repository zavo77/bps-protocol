...
const DIR = "C:/Projects/bps-experiment/packages/contracts/canary-packet/exec";
...
async function runImmediate(record){
  ...
  for(let i=0;i<8;i++){
    ...
    // record the ACTUAL deployed runtime code hash (includes constructor-set immutables) for exact restart verification
    deployedRuntimeHashes[slots[i]]=keccak256(await rpc("eth_getCode",[getAddress(P[slots[i]]),"latest"]));
  ...
  const buyAcct={tradeId:hx(buyEv.tradeId),trader:getAddress(buyEv.trader),recipient:getAddress(buyEv.recipient),adapter:getAddress(buyEv.adapter),stockBudgetRecipient:getAddress(buyEv.stockBudgetRecipient),
    grossWethInput:hx(BUY_WETH_WEI),grossWethMicroUsd:hx(usdMicroOfWei(BUY_WETH_WEI)),stockBudget2pct:hx(buyEv.stockBudget),burnBudget1pct:hx(buyEv.burnBudget),userWethBudget97pct:hx(buyEv.userWethBudget),
  ...
  const sellAcct={tradeId:hx(sellEv.tradeId),trader:getAddress(sellEv.trader),recipient:getAddress(sellEv.recipient),adapter:getAddress(sellEv.adapter),stockBudgetRecipient:getAddress(sellEv.stockBudgetRecipient),
    grossBpsInput:hx(sellBps),grossWethOutput:hx(sellEv.grossWethOutput),stockAcquisition2pct:hx(sellEv.stockBudget),retirementBurn2pct:hx(sellEv.burnBudget),userWethOutput96pct:hx(sellEv.userWethOutput),bpsBurned:hx(sellEv.bpsBurned),
  ...
  let lockId,principal,unlockTime,lockAccount,lockStart,lockDuration,lockMult,lockPolicy; for(const lg of lockRc.logs){try{const d=decodeEventLog({abi:lockAbi,data:lg.data,topics:lg.topics});if(d.eventName==="LockCreated"){lockId=d.args.lockId;principal=d.args.principal;unlockTime=d.args.unlockTime;lockAccount=d.args.account;lockStart=d.args.startTime;lockDuration=d.args.duration;lockMult=d.args.multiplierBps;lockPolicy=d.args.policyVersion;}}catch{}}
  ...
  return { txs, P, pool:getAddress(pool), canary, vault, router, locking, claimManager:P.claimManager, rialtoAdapter:P.rialtoAdapter, coordinator:P.coordinator, uniswapAdapter:P.uniswapAdapter,
    lpUsage, buyAcct, sellAcct, lockId, principal, unlockTime, lockAccount, lockStart, lockDuration, lockMult, lockPolicy, token0, token1, deployedRuntimeHashes };
}
...
async function main(){
  ...
  const snapForSlot=JSON.parse(readFileSync(`${DIR}/block-snapshot.json`,"utf8"));
  ...
  // ---- safety / authorization flags ----
  // execution mode: flags come from policy (may be all-true after the operator's live authorization);
  // non-execution mode: flags MUST be all-false. Neither path weakens any integrity/replay/exposure check.
  const EXEC_MODE = policy.authorizationMode === "execution";
  const flagKeys=["broadcastReady","liveWritesApproved","executionAuthorized","fundingAuthorized"];
  const safetyFlags={}; for(const k of flagKeys) safetyFlags[k]=policy.safetyFlags[k]===true;
  if(EXEC_MODE){
    for(const k of flagKeys) if(policy.safetyFlags[k]!==true) throw new Error("execution mode requires policy flag true: "+k);
    if(policy.executable!==true) throw new Error("execution mode requires policy.executable=true");
    if(policy.production!==false || policy.canary!==true) throw new Error("execution packet must be canary=true, production=false");
    if(Number(policy.maxAllInclusiveExposureUsd)!==130) throw new Error("maxAllInclusiveExposureUsd must be 130");
    if(!policy.authorizationScope) throw new Error("execution mode requires authorizationScope");
  } else {
    for(const k of flagKeys) if(policy.safetyFlags[k]!==false) throw new Error("policy flag not false: "+k);
  }
  const authorization = EXEC_MODE ? {
    executionAuthorized:true, fundingAuthorized:true, liveWritesApproved:true, broadcastReady:true, executable:true,
    canary:true, production:false, chainId:CHAIN_ID, maxAllInclusiveExposureUsd:130, mustRegenerateBeforeExecution:false,
    scope:policy.authorizationScope, signingChannel:"user Rabby extension via per-transaction eth_sendTransaction only (no key handling in operator or packet)",
    delayedWithdrawalDisabled:"tester nonce 8 withdraw requires a separately regenerated packet + fresh preflight after unlock; NOT executable from the immediate operator",
  } : null;
  ...
  // ---- assemble ----
  const artifact={
    meta:{ task:EXEC_MODE?"10D-exec":"10B-8", kind:EXEC_MODE?"EXECUTION-ENABLED BPSC-TEST canary packet (localhost Rabby operator; per-tx user approval; NO key handling)":"self-contained replayable canary review packet — HISTORICAL FIXTURE (non-live, non-executable)",
      mode:policy.mode, live:policy.live===true, executable:policy.executable===true, executionAuthorized:safetyFlags.executionAuthorized, fundingAuthorized:safetyFlags.fundingAuthorized,
      authorization,
      fixtureNote:EXEC_MODE
        ? "EXECUTION-ENABLED. Signing occurs ONLY in the user Rabby extension via per-transaction eth_sendTransaction after an explicit click. This packet was PREPARED without any signing or broadcast."
        : (policy.mode==="live" ? "TASK 10C: FRESH LIVE unsigned packet; preparation only." : "Historical test fixture."),
      ...
      simulationTimestamp:SIM_TS, immediatePacketDeadline:hx(DEADLINE), snapshotBound:true, expiry:hx(DEADLINE), mustRegenerateBeforeExecution:policy.mustRegenerateBeforeExecution!==false,
    ...
    fundingPlan, roleConfigurationProof:roleProof, capArithmetic, addressGuards, deployedRuntimeCodeHashes:R.deployedRuntimeHashes, lpUsage:R.lpUsage, buyAccounting:R.buyAcct, sellAccounting:R.sellAcct,
    lockAndWithdraw:{ account:getAddress(R.lockAccount), lockId:hx(R.lockId), principal:hx(R.principal), startTime:hx(R.lockStart), duration:Number(R.lockDuration), unlockTime:hx(R.unlockTime), multiplierBps:Number(R.lockMult), policyVersion:Number(R.lockPolicy), returnedPrincipal:delayed.returnedPrincipal, returnedEqualsPrincipal:delayed.returnedEqualsPrincipal },
  ...
  // ---- TASK 10D-3: ONE canonical reviewed-authorization object + independent digest anchoring ----
  // Build the canonical object over EVERY security-relevant leaf (execDigest + deploymentOnlyDigest are set
  // above; fullReviewArtifactDigest is intentionally excluded to avoid a cycle). Anchor its digest in the
  // packet, the policy, AND the packaged operator (operator/reviewed-authorization.json). At binding time
  // the browser reconstructs this object from the runtime inputs and requires all three anchors to agree.
  const snapForCanon=JSON.parse(readFileSync(`${DIR}/block-snapshot.json`,"utf8"));
  const canon=buildCanonical({packet:artifact, policy, snapshot:snapForCanon, getAddress});
  if(canon.errors.length) throw new Error("canonical reviewed-authorization build failed: "+canon.errors.join("; "));
  const reviewedAuthorizationDigest=digestOf(canon.object, keccak256);
  artifact.meta.reviewedAuthorizationDigest=reviewedAuthorizationDigest;
  artifact.meta.reviewedAuthorizationDigestDomain="keccak256( canonicalStable( buildCanonical(packet,policy,snapshot) ) ) — every security-relevant leaf; equality required across packet.meta, review-policy, and operator/reviewed-authorization.json before Connect";

  // fullReviewArtifactDigest: over the NORMALIZED artifact minus meta.fullReviewArtifactDigest. Computed AFTER
  // reviewedAuthorizationDigest is set so the full digest also covers it.
  ...
  // policy anchor
  policy.reviewedAuthorizationDigest=reviewedAuthorizationDigest;
  writeFileSync(`${DIR}/review-policy.json`, JSON.stringify(policy,null,2));
  // packaged operator anchor (the single bound runtime data source)
  writeFileSync(`${DIR}/operator/reviewed-authorization.json`, JSON.stringify(canon.object,null,2));
  ...
  try {
    const bmPath=`${DIR}/manifests/robinhood-mainnet.canary.json`;
  ...
  console.log("reviewedAuthorizationDigest:",reviewedAuthorizationDigest);
