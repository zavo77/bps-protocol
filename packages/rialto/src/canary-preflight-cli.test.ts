...
import { runCanaryPreflightCli, type CliDeps } from "./canary-preflight-cli.js";
import {
  type ChainReader,
  ETH_USD_FEED,
...
function deps(over: Partial<CliDeps>): CliDeps {
  ...
  // expose captured lines for assertions
...
describe("runCanaryPreflightCli — evaluation", () => {
  ...
  it("aborts gracefully (exit 1) if the reader throws", async () => {
    const d = deps({
      makeReader: () => ({
        getChainId: async () => {
    ...
    const out = await runCanaryPreflightCli(d);
    // getChainId failure is captured as a failed check, not a throw.
    expect(out.status).toBe("CANARY_NOT_READY");
  });
});
