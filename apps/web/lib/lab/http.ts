...
export function mapError(e: unknown): NextResponse {
  ...
  if (msg.startsWith('Missing required environment')) return err('ENV_MISSING', 'Server configuration incomplete.', 503);
  if (msg.startsWith('ANCHOR_MISMATCH')) return err('ANCHOR_MISMATCH', msg, 409);
  if (msg === 'FEE_PRESET_DISABLED') return err(msg, 'That fee preset is not available on this chain.', 400);
  if (msg === 'BPS_BENEFICIARY_UNCONFIGURED') return err(msg, 'Server configuration incomplete.', 503);
