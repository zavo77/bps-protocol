...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    if (rateLimited(`simulate:${clientKey(req)}`, 10)) return err('RATE_LIMITED', 'Too many requests.', 429);
    ...
    return ok({ simulation: bundle.simulation, prepared: bundle.prepared, manifestHash: bundle.manifestHash });
  } catch (e) {
    return mapError(e);
  }
}
