...
| Last completed milestone    | **TASK 10K-1A — corrected the GET /quote eval harness units + governance** (`packages/rialto/src/quote-eval-cli.ts`, `quote-client.ts`): `sell_amount` is HUMAN-DECIMAL (`RIALTO_SELL_AMOUNT_DECIMAL`); official origin `https://rialto-trade-api.rialto.xyz` enforced (fail-closed); expanded sanitized report; exception renamed **QEX-1** (D-2 read-only preference PRESERVED, currently unavailable via Rialto dashboard — bounded exception, not an erasure). rialto vitest 129/129. Prior: 10K-1 (`1d580e3`) |
| Current active task         | TASK 10K-1A harness corrected + verified offline; **no live GET /quote run yet** — blocked on runtime inputs (taker address, `RIALTO_SELL_AMOUNT_DECIMAL`, slippage) and a rotated key (the pasted key is exposed in-transcript and should be revoked). Official origin is the built-in default                                                                                                                                                                                                                    |
| Exact next recommended task | Founder revokes the exposed key + issues a fresh bundled key, runs the harness locally via the masked secret-entry method (§P; key stays in the founder env) with real inputs, and returns the sanitized report; then reconcile `tx.to` against the official router registry before any D-6 pinning. D-24 stands — no acquisition/execution                                                                                                                                                                        |
