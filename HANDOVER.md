...
| Last completed milestone    | **TASK 10K-4 — Solidity `GuardedSettlementExecutor` + offline Foundry tests** (`packages/contracts/src/GuardedSettlementExecutor.sol`, undeployed, paused/disabled by default). Ownable2Step Safe controller, executor-as-taker, registry `ownerOf(2)` lock, per-selector calldata validator, price guard (D-22B), 0.01 WETH cap, exact-allowance + min-delta + allowance-reset, domain digest + replay. `0x77963966` hard-blocked. forge-std v1.9.7 (offline). 50 Foundry tests (unit/fuzz/invariant) + TS↔Solidity digest parity; rialto vitest 202/202. Prior: 10K-3 (`ac70118`) |
| Current active task         | none — Solidity executor implemented + tested locally (offline); UNDEPLOYED; live trading NOT enabled; D-24 stands                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Exact next recommended task | Resolve the gates blocking the first controlled canary, in order: (1) **authoritative selector proof** for `0x77963966` from a repo-owned/vendored Rialto router ABI (D-6; hard-blocked until proven) + its specific validator; (2) trusted **price source** + freshness (D-22B); (3) production **Safe** controller address; (4) **dated runtime registry** strategy (D-6); (5) per-token cap approval (D-8); (6) deployment script/procedure + counsel (D-3/D-23) + a new bounded founder authorization (D-24). Nothing here is deployed or authorized                            |
...
- **`packages/rialto`** — server-only Rialto boundary: quote client (`src/quote-client.ts`), structural
  ...
  **retired** and now fails with `QEX1_CONSUMED` before any env read or network call (no env override
  ...
  disabled (empty production selector list, unresolved taker/caps/ages/price source) and fails closed. A
  contract-ready Solidity interface (`packages/contracts/src/interfaces/IGuardedSettlementExecutor.sol`)
  mirrors these invariants; the concrete executor is deferred (forge-std absent offline).
  Production terms + counsel gates remain before any acquisition (D-24 stands).
