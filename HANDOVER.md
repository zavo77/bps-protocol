...
| Last completed milestone    | **TASK 10K-3 — guarded-settlement core (production-shaped, deliberately DISABLED)**. Pure offline `packages/rialto/src/guarded-settlement.ts` (typed policy, domain-separated intent digest, registry/selector/taker/token/amount/fee/value/price/replay guards, sanitized settlement plan, `READY_OFFLINE_ONLY`); fails closed on unresolved production facts. Doc: `docs/audit/BPS_RIALTO_GUARDED_SETTLEMENT_2026-07-25.md`. Solidity executor DEFERRED (forge-std absent offline); interface `packages/contracts/src/interfaces/IGuardedSettlementExecutor.sol`. rialto vitest 201/201. Prior: 10K-2 (`46c9855`) |
| Current active task         | none — guarded core implemented offline + verified; live trading NOT enabled; D-24 stands                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Exact next recommended task | Resolve the production gates blocking a private controlled canary, in order: (1) approved selector proven from a repo-owned/vendored router ABI (D-6); (2) final guarded-executor/Safe taker (D-6); (3) dated runtime `ownerOf(2)` strategy (D-6); (4) trusted price source + age (D-22B); (5) per-token sell cap (D-8); (6) vendored forge-std → concrete `IGuardedSettlementExecutor` + tests (D-21/D-5). D-24 stands — no acquisition/execution                                                                                                                                                                  |
