...
| Last completed milestone    | **TASK 10K-1 — isolated non-executing Rialto GET /quote evaluation harness** (`packages/rialto/src/quote-eval-cli.ts`) + recorded founder **Quote-Evaluation Exception** (ballot D-2 superseded: bundled key usable for GET /quote testing only, no execution). rialto vitest 107/107. Prior: 10J-3 oracle/feed evidence + D-1..D-24 register |
| Current active task         | TASK 10K-1 harness built + verified offline; **no live GET /quote run yet** — blocked on runtime inputs (Rialto API base URL, taker address, sell amount, slippage) and a rotated key (the pasted key is exposed in-transcript and should be revoked)                                                                                         |
| Exact next recommended task | Founder revokes the exposed key + issues a fresh bundled key, runs the harness locally (key stays in the founder env) with real inputs, and returns the sanitized report; then pin the venue selector (D-6) from the observed calldata. D-24 stands — no acquisition/execution                                                                |
...
- **`packages/rialto`** — server-only Rialto boundary: quote client (`src/quote-client.ts`), structural
  ...
  (`src/boundary-status.ts`), and an isolated **non-executing** GET /quote eval harness
  (`src/quote-eval-cli.ts`). Key is read from `RIALTO_API_KEY` (never logged/returned). Under the founder
  **Quote-Evaluation Exception (2026-07-25)** the harness may issue **GET /quote only** — no signing,
  funding, allowance, Permit2, or submission; production terms + counsel gates remain before any acquisition.
...
| D-017 | 2026-07-25  | **Quote-Evaluation Exception**: ballot D-2 (quote:read-only key) superseded — Rialto issues no read-only key, so the mandatory bundled key may be used **only** in an isolated, non-executing quote-evaluation environment (no signing key, funded wallet, allowance, Permit2 signature, or swap-submission route); authorizes **GET /quote testing only**; does NOT authorize acquisition/execution (D-24 stands) | DECIDED (founder) / IMPLEMENTED (harness) | Rialto boundary, ops     | ballot D-2 (SUPERSEDED)              |
...
- **2026-07-25 (TASK 10K-1 — isolated non-executing Rialto GET /quote evaluation harness + Quote-Evaluation
  Exception)** — Founder issued a **Quote-Evaluation Exception** superseding ballot D-2: Rialto confirmed it
  ...
  revoked). Recorded as decision **D-017**. Commit `feat(rialto): isolated non-executing quote-eval harness`.
