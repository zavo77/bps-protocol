...
| Last completed milestone    | **TASK 10K-5 — removed the Rialto-ABI blocker via OPAQUE-CALL validation** (`packages/contracts/src/GuardedSettlementExecutor.sol`, undeployed, paused/disabled). Executor now treats `tx.data` as opaque and binds an **approved runtime code hash + selector** instead of decoding an ABI (per-selector calldata validator removed). On-chain investigation (block 19674173): router code hash `0xa7041268…27611` (direct impl, not a proxy), `ownerOf(2)`==router, selector `0x77963966` in dispatcher, router = target + allowance spender. 55 Foundry tests (incl. malicious routers + invariant) + TS opaque quote-validation boundary; rialto vitest 219/219. Prior: 10K-4 (`164bec0`) |
| Current active task         | none — opaque-call executor implemented + tested locally (offline); UNDEPLOYED; live trading NOT enabled; D-24 stands                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Exact next recommended task | The missing Rialto ABI is NO LONGER a blocker. First-canary gates: (1) approve `0x77963966` **paired with** the observed router code hash `0xa7041268…27611`; (2) trusted **price source** + freshness (D-22B); (3) production **Safe** controller/taker + activation (D-6); (4) per-token cap approval (D-8); (5) deployment script/procedure + external audit + counsel (D-3/D-23) + a new bounded founder authorization (D-24). Nothing here is deployed or authorized                                                                                                                                                                                                                     |
